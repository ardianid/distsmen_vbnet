...
Public Class tr_kas_in
    ...
    Private Sub opendata()
        ...
        If get_stat_pusat() = 1 Then
            ...
            sql = "SELECT A.NO_BUKTI,A.TANGGAL,B.NAMA,A.KETERANGAN,A.TOTAL,A.KD_KAS FROM ADMLSP.TR_KAS_IN A INNER JOIN ADMLSP.MS_KAS B ON A.KD_KAS=B.KODE " & _
                "WHERE ROWNUM <=50 ORDER BY A.TANGGAL DESC"

        ElseIf get_stat_pusat() = 0 Then

            sql = String.Format("SELECT A.NO_BUKTI,A.TANGGAL,B.NAMA,A.KETERANGAN,A.TOTAL,A.KD_KAS FROM ADMLSP.TR_KAS_IN A INNER JOIN ADMLSP.MS_KAS B ON A.KD_KAS=B.KODE " & _
    ...
    Private Sub hapus_data()
        ...
        If dv1.Count > 0 Then
            ...
            Dim sql As String = String.Format("delete from admlsp.TR_KAS_IN where no_bukti='{0}'", dv1(BindingContext(bs1).Position)("NO_BUKTI").ToString)
            Dim sqlupdate As String = String.Format("update admlsp.TR_KAS_IN set user_edit='{0}',tgl_edit='{1}' where no_bukti='{2}'", userprog, convert_date_to_eng(Date.Now.ToString), dv1(BindingContext(bs1).Position)("NO_BUKTI").ToString)
            ...
            Try
                ...
                If MsgBox("Yakin akan dihapus ? ", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "Konfirmasi") = MsgBoxResult.Yes Then
                    ...
                    ins_to_temp_user(cn, sqlupdate, dv1(BindingContext(bs1).Position)("NO_BUKTI").ToString, "TR_KAS_IN", "UPDATE", "KAS MASUK")

                    cmd = New OracleCommand(sql, cn)
                    cmd.ExecuteNonQuery()


                    ins_to_temp_user(cn, sql, dv1(BindingContext(bs1).Position)("NO_BUKTI").ToString, "TR_KAS_IN", "DELETE", "KAS MASUK")
    ...
    Private Sub tr_kasin_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub tsadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsadd.Click
        ...
        Using tr_dep2 As New tr_kas_in2(True, dv1, 0) With {.StartPosition = FormStartPosition.CenterScreen}
    ...
    Private Sub tsedit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsedit.Click
        ...
        Using tr_dep2 As New tr_kas_in2(False, dv1, bs1.Position) With {.StartPosition = FormStartPosition.CenterScreen}
            tr_dep2.ShowDialog(Me)
        End Using
