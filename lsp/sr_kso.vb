...
Public Class sr_kso
    ...
    Private Sub opendata()
        ...
        sql = "SELECT A.KODE,A.NAMA,A.ALAMAT FROM ADMLSP.MS_KSO A WHERE ROWNUM <= 1000 AND A.AKTIF=1 ORDER BY A.KODE"
    ...
    Private Sub cari()
        ...
        Select Case cbkriteria.SelectedIndex
            Case 0
                ...
                sql = String.Format("SELECT A.KODE,A.NAMA,A.ALAMAT FROM ADMLSP.MS_KSO A WHERE ROWNUM <= 1000 AND A.AKTIF=1 AND A.KODE LIKE '%{0}%' ORDER BY A.KODE", tcari.Text.Trim.ToUpper)

            Case 1

                sql = String.Format("SELECT A.KODE,A.NAMA,A.ALAMAT FROM ADMLSP.MS_KSO A WHERE ROWNUM <= 1000 AND A.AKTIF=1 AND A.NAMA LIKE '%{0}%' ORDER BY A.KODE", tcari.Text.Trim.ToUpper)
