...
Public Class pr_gaji_sal_tot
    ...
    Private Sub open_data()
        ...
        sql = String.Format("SELECT V_GAJI_SALES.NODO, V_GAJI_SALES.TANGGAL, V_GAJI_SALES.NAMA_TOKO, V_GAJI_SALES.TOT_AKH, V_GAJI_SALES.TOT_INSENTIF, V_GAJI_SALES.NAMA_SALES, V_GAJI_SALES.KECAMATAN " & _
        ...
        sql = String.Format("SELECT V_GAJI_SALES_TOT.KD_SALES, V_GAJI_SALES_TOT.NAMA_SALES, V_GAJI_SALES_TOT.TOT_AKH, V_GAJI_SALES_TOT.TOT_INSENTIF, V_GAJI_SALES_TOT.BLN, V_GAJI_SALES_TOT.THN " & _
                "FROM   ADMLSP.V_GAJI_SALES_TOT V_GAJI_SALES_TOT " & _
                 "WHERE V_GAJI_SALES_TOT.BLN='{0}' and V_GAJI_SALES_TOT.THN='{1}'", bln, thn)


        If Not (kdarea.Trim.ToUpper = "ALL") Then
            sql = String.Format(" {0} AND V_GAJI_SALES_TOT.KD_STAT='{1}'", sql, kdarea)
        End If

        If kodekaryawan.Trim.Length > 0 Then
            sql = String.Format(" {0} AND V_GAJI_SALES_TOT.KD_SALES='{1}'", sql, kodekaryawan)
        End If

        sql = String.Format(" {0} {1}", sql, "ORDER BY V_GAJI_SALES_TOT.THN, V_GAJI_SALES_TOT.BLN")
        ...
        Try
            ...
            Dim objRpt As New r_gaji_sales_tot
